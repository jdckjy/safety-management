...
interface Props {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (newTenant: Omit<TenantInfo, 'id'>) => void;
}

const initialFormData: Omit<TenantInfo, 'id'> = {
  companyName: '',
...
export const AddTenantInfoModal: React.FC<Props> = ({ isOpen, onClose, onAdd }) => {
  const [formData, setFormData] = useState<Omit<TenantInfo, 'id'>>(initialFormData);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[625px] bg-white p-6">
        <DialogHeader>
          <DialogTitle className="text-lg font-semibold">임차인 추가</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 py-6">
            ...
            <div className="space-y-2">
              ...
              <Select onValueChange={(value) => handleSelectChange('businessCategory', value)} defaultValue={formData.businessCategory}>
            ...
            <div className="space-y-2">
              ...
              <Select onValueChange={(value) => handleSelectChange('companySize', value)} defaultValue={formData.companySize}>
                <SelectTrigger className="bg-gray-50">
          ...
          <DialogFooter className="gap-x-2">
            ...
            <Button type="submit" size="lg">추가</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
