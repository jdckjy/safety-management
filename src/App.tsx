...
// Storage and baseline constants remain the same...
const STORAGE_KEYS = {
...
const App: React.FC = () => {
    ...
    const fetchTasks = async () => {
        ...
        const tasksList = tasksSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as TaskItem));
        setTasks(tasksList);
    };

    const fetchHotspots = async () => {
        const hotspotsCollection = collection(db, 'hotspots');
        const hotspotsSnapshot = await getDocs(hotspotsCollection);
        const hotspotsList = hotspotsSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as HotSpot));
    ...
    // CRUD Handlers for Hotspots
    const handleAddHotspot = async (newHotspotData: Omit<HotSpot, 'id'>) => {
        await addDoc(collection(db, 'hotspots'), newHotspotData);
        fetchHotspots();
    };
    const handleUpdateHotspot = async (updatedHotspot: HotSpot) => {
        const { id, ...dataToUpdate } = updatedHotspot;
        await updateDoc(doc(db, 'hotspots', id), dataToUpdate);
        fetchHotspots();
    };
    ...
    const renderContent = () => {
        ...
        switch (activeMenu) {
            case 'dashboard': return <Dashboard tasks={tasks} onTasksUpdate={fetchTasks} summaryStats={summaryStats} selectedMonth={selectedMonth} />;
            case 'safety': return <SafetyManagement kpis={safetyKPIs} onUpdate={setSafetyKPIs} mainValue={summaryStats.safety} facilities={facilities} hotspots={hotspots} onAddHotspot={handleAddHotspot} onUpdateHotspot={handleUpdateHotspot} onDeleteHotspot={handleDeleteHotspot} />;
            ...
            default: return <Dashboard tasks={tasks} onTasksUpdate={fetchTasks} summaryStats={summaryStats} selectedMonth={selectedMonth} />;
        }
    };
    ...
    return (
        <div className="flex h-screen w-full bg-[#F8F7F4] text-[#1A1D1F] font-sans">
            ...
            <main className="flex-1 flex flex-col overflow-hidden p-6 md:px-12 md:py-6 space-y-4">
                <Header activeMenu={activeMenu} customTabs={customTabs} selectedMonth={selectedMonth} onMonthChange={setSelectedMonth} />
