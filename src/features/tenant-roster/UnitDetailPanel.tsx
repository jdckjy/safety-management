...
const UnitDetailPanel: React.FC<UnitDetailPanelProps> = ({ unit, onEdit, onDelete, onEditTenant }) => {
  ...
  const tenantName = tenant ? (tenant.companyName || tenant.businessName) : '-';
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg h-full flex flex-col">
      ...
      <dl className="flex-grow p-6 overflow-y-auto">
        ...
        {tenant && contract && (
          <>
            {renderDetail('주요 사업', tenant.businessCategory || tenant.businessType)}
            {renderDetail(
              '상주 인원',
            ...
            {renderDetail('월 임대료', contract.rent ? `${contract.rent.toLocaleString()} 원` : '-')}
