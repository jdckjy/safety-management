...
const kosisApiKey = defineString("KOSIS_API_KEY");

const corsHandler = cors({ origin: true });

export const kosisProxy = functions.https.onRequest((request, response) => {
  corsHandler(request, response, async () => {
    ...
    try {
      ...
      const params = {
        ...
        itmId: "T20",
        prdSe: "M",
        startPrdDe: startPrdDe as string,
        endPrdDe: endPrdDe as string,
        userStatsId: request.query.userStatsId || 'jdckjy/101/DT_1B040A3/2/1/20240616142718'
      };
      ...
      const kosisResponse = await axios.get(kosisUrl, { headers, params, timeout: 10000 });
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.code === 'ECONNRESET') {
           ...
           const data = axiosError.response?.data || axiosError.message || "An unexpected error occurred while communicating with the KOSIS API.";
