...
async function migrateUsers(db) {
    ...
    const userDocsSnap = await (0, firestore_1.getDocs)(usersRef);
    ...
    for (const userDoc of userDocsSnap.docs) {
        ...
        const cleanedTenants = (userData.tenantInfo || []).filter((tenant) => !MOCK_TENANT_IDS.includes(tenant.id));
        ...
        const cleanedContracts = (userData.contracts || []).filter((contract) => !MOCK_TENANT_IDS.includes(contract.tenantId));
